...
class 基础行为测试(unittest.TestCase):
    ...
    def test_register_and_find(self):
        ...
        saved = service.register({"record_id": "exhibit-001", "owner_id": "curatorial-office", "state": "draft", "revision": 1})
        self.assertEqual(saved["revision"], 1)
        self.assertEqual(service.find(saved["record_id"])["owner_id"], saved["owner_id"])

    def test_duplicate_is_rejected(self):
        service = Service(Store())
        payload = {"record_id": "exhibit-001", "owner_id": "curatorial-office", "state": "draft", "revision": 1}
